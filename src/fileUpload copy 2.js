...
function FileUpload() {
  ...
  const [showLoading, setShowLoading] = useState(false); // 로딩 중 표시 상태
  const [showButtons, setShowButtons] = useState(false); // 결과 버튼 표시 상태
  ...
  const handleFileUpload = (e) => {
    ...
    setShowLoading(false); // 로딩 중 표시 초기화
    setShowButtons(false); // 결과 버튼 초기화
  ...
  const handleAnalyze = async() => {
    ...
    try {
      ...
      setShowLoading(false); // 로딩 중 표시 종료
      if (data.success) {
    ...
    <div className="file-upload-container">
      <div className="left-section">
        <div className="upload-section">
          ...
          {/* 파일이 없을 때도 미리보기 박스를 회색으로 보여줍니다 */}
          <div className="image-preview">
          {/* 업로드된 파일이 없으면 기본 회색 박스, 있으면 미리보기 이미지 */}
          {preview ? (
      ...
      {showLoading && <div className="loading-message">로딩 중...</div>}
        ...
        {/* 분석 실패 시 "인식불가" 버튼 */}
        {!loading &&  !showLoading && (
          <div className={`unrecognized-button ${error ? 'blinking' : ''}`}>
            인식불가
